        ...
        <table className="w-full min-w-[880px] text-sm">
          <thead className="bg-muted/50 text-muted-foreground">
            <tr>
              ...
              <th className="px-4 py-2 text-start font-medium">{t("wifiName")}</th>
              <th className="px-4 py-2 text-start font-medium">{t("wifiPass")}</th>
          ...
          <tbody>
            {bills.map((b) => (
              <tr key={b.apartmentId} className="border-t border-border">
                ...
                <td className="px-4 py-3">{can.viewWifi(user.role) ? b.wifiName : "••••"}</td>
                <td className="px-4 py-3 font-mono text-xs">{can.viewWifi(user.role) ? b.wifiPassword : t("accountSensitive")}</td>
                ...
                <td className="px-4 py-3">
                  {b.status !== "paid" && can.payBills(user.role) ? (
                    <Button
                      ...
                      variant="outline"
                      onClick={() => {
                        markUtilityPaid("internet", b.apartmentId);
                        toast.success(`${aptName(data, b.apartmentId)} — Internet Bill — ${money(b.amount, lang)} — ${t("billPaid")} ✅`);
                      }}
                    >
                      {t("markPaid")}
                    </Button>
